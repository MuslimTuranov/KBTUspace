...
export default function FeedPage() {
  ...
  const [selectedFacultyId, setSelectedFacultyId] = useState<number | null>(user?.role === 'admin' ? null : user?.faculty_id ?? null);
  const [showCreate, setShowCreate] = useState(false);
  const canCreate = Boolean(user);
  const { data: posts, isLoading, error } = useQuery({
    queryKey: ['posts', filter, selectedFacultyId],
    queryFn: () => getPosts(filter === 'global' ? { global: true } : { faculty_id: selectedFacultyId ?? undefined }),
    enabled: filter === 'global' || selectedFacultyId !== null,
  });
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        {canCreate && <button onClick={() => setShowCreate(true)} className="btn-primary"><Plus className="w-4 h-4" /> New Post</button>}
      </div>
      <div className="flex flex-wrap items-center gap-2 mb-6">
        ...
        {filter === 'faculty' && (
          <select value={selectedFacultyId ?? ''} onChange={(e) => setSelectedFacultyId(e.target.value ? Number(e.target.value) : null)} className="input py-2 text-sm max-w-xs">
      ...
      {showCreate && canCreate && <CreatePostModal onClose={() => setShowCreate(false)} />}
    </div>
  );
}
