...
export default function EventsPage() {
  ...
  const { data: events, isLoading, error } = useQuery({
    queryKey: ['events', filter, selectedFacultyId],
    queryFn: () => getEvents(filter === 'global' ? { global: true } : { faculty_id: selectedFacultyId ?? undefined }),
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        {canCreate && <button onClick={() => setShowCreate(true)} className="btn-primary"><Plus className="w-4 h-4" /> New Event</button>}
      </div>
