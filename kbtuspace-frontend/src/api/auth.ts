...
export interface RegisterRequest { email: string; password: string; faculty_id: number; }
...
export const changePassword = async (data: { current_password: string; new_password: string }) => { const res = await api.patch<{ message: string }>('/profile/password', data); return res.data; };
